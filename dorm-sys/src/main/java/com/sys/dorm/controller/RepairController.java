/**
 * @Copyright:           Copyright© 2009-2016 UXUNCHINA
 * @Company:             深圳市优讯信息技术有限公司
 * @Project:             dorm-sys
 * @see                  com.sys.dorm.controller                              
 */
package com.sys.dorm.controller;

import java.util.List;
import java.util.Map;

import javax.validation.Validator;

import org.apache.commons.lang3.StringUtils;
import org.apache.shiro.authz.annotation.RequiresPermissions;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.ModelAttribute;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RequestMethod;
import org.springframework.web.bind.annotation.RequestParam;
import org.springframework.web.bind.annotation.ResponseBody;
import org.springside.modules.beanvalidator.BeanValidators;

import com.sys.dorm.entity.main.Repair;
import com.sys.dorm.exception.ExistedException;
import com.sys.dorm.exception.ServiceException;
import com.sys.dorm.service.RepairService;
import com.sys.dorm.util.dwz.AjaxObject;
import com.sys.dorm.util.dwz.Page;

/**
 * <p>
 * <b>Title: </b>ViolationController.java
 * <p>
 * <b>Description: </b>TODO
 * 
 * @author Xupk
 * @version V1.0
 *          <p>
 *          2016年4月23日 Xupk 创建类 <br>
 * 
 */

@Controller
@RequestMapping("/management/security/repair")
public class RepairController {

	@Autowired
	private Validator validator;

	@Autowired
	private RepairService repairService;

	private static final String CREATE = "management/security/repair/create";
	private static final String UPDATE = "management/security/repair/update";
	private static final String LIST = "management/security/repair/list";
	private static final String NEW = "management/security/repair/new";

	@RequiresPermissions("Repair:view")
	@RequestMapping(value = "/list", method = { RequestMethod.GET,
			RequestMethod.POST })
	public String list(Page page, String keywords, Map<String, Object> map) {
		List<Repair> violations;
		if (StringUtils.isNotBlank(keywords)) {
			violations = repairService.find(page, keywords);
		} else {
			violations = repairService.findAll(page);
		}

		map.put("page", page);
		map.put("violations", violations);
		map.put("keywords", keywords);
		return LIST;
	}

	@RequiresPermissions("Create:save")
	@RequestMapping(value = "/new", method = RequestMethod.GET)
	public String preNew() {
		return NEW;
	}
	
	
	@RequiresPermissions("Create:save")
	@RequestMapping(value = "/new", method = RequestMethod.POST)
	public @ResponseBody
	String newCreate(Repair repair) {
		BeanValidators.validateWithException(validator, repair);
		AjaxObject ajaxObject = new AjaxObject();
		try {
			repairService.save(repair);
		} catch (ExistedException e) {
			ajaxObject = new AjaxObject(e.getMessage());
			ajaxObject.setCallbackType("");
			ajaxObject.setStatusCode(AjaxObject.STATUS_CODE_FAILURE);
			return ajaxObject.toString();
		}
		ajaxObject.setCallbackType("");
		return AjaxObject.newOk("添加记录成功！").toString();
	}
	
	@RequiresPermissions("Repair:save")
	@RequestMapping(value = "/create", method = RequestMethod.GET)
	public String preCreate() {
		return CREATE;
	}

	@RequiresPermissions("Repair:save")
	@RequestMapping(value = "/create", method = RequestMethod.POST)
	public @ResponseBody
	String create(Repair repair) {
		BeanValidators.validateWithException(validator, repair);

		try {
			repairService.save(repair);
		} catch (ExistedException e) {
			AjaxObject ajaxObject = new AjaxObject(e.getMessage());
			ajaxObject.setCallbackType("");
			ajaxObject.setStatusCode(AjaxObject.STATUS_CODE_FAILURE);
			return ajaxObject.toString();
		}
		return AjaxObject.newOk("添加记录成功！").toString();
	}

	@ModelAttribute("preloadRepair")
	public Repair getOne(
			@RequestParam(value = "id", required = false) Long id) {
		if (id != null) {
			Repair repair = repairService.get(id);
			return repair;
		}
		return null;
	}

	@RequiresPermissions("Repair:edit")
	@RequestMapping(value = "/update/{id}", method = RequestMethod.GET)
	public String preUpdate(@PathVariable Long id, Map<String, Object> map) {
		Repair repair = repairService.get(id);

		map.put("repair", repair);
		return UPDATE;
	}

	@RequiresPermissions("Repair:edit")
	@RequestMapping(value = "/update", method = RequestMethod.POST)
	public @ResponseBody
	String update(@ModelAttribute("preloadRepair") Repair repair) {
		BeanValidators.validateWithException(validator, repair);
		repairService.update(repair);

		return AjaxObject.newOk("修改记录成功！").toString();
	}

	@RequiresPermissions("Repair:delete")
	@RequestMapping(value = "/delete/{id}", method = RequestMethod.POST)
	public @ResponseBody
	String delete(@PathVariable Long id) {
		AjaxObject ajaxObject = new AjaxObject("删除记录成功！");
		try {
			repairService.delete(id);
		} catch (ServiceException e) {
			ajaxObject.setMessage(e.getMessage());
		}

		ajaxObject.setCallbackType("");
		return ajaxObject.toString();
	}

	@RequiresPermissions("Repair:reset")
	@RequestMapping(value = "/reset/status/{id}", method = RequestMethod.POST)
	public @ResponseBody
	String reset(@PathVariable Long id) {
		Repair repair = repairService.get(id);
		AjaxObject ajaxObject = new AjaxObject();
		if (repair.getStatus().equals("enabled")) {
			repair.setStatus("disabled");
		} else {
			repair.setStatus("enabled");
		}

		ajaxObject.setMessage("修改状态成功，当前为"
				+ (repair.getStatus().equals("enabled") ? "未处理" : "已处理"));

		repairService.update(repair);
		ajaxObject.setCallbackType("");
		return ajaxObject.toString();
	}

}
